...
#     Class that implements a gym.Env interface for traffic signal control using the SUMO simulator.
#     See [https://sumo.dlr.de/docs/](https://sumo.dlr.de/docs/) for details on SUMO.
#     See [https://gymnasium.farama.org/](https://gymnasium.farama.org/) for details on gymnasium.

#     Args:
#         net_file (Optional[str]): SUMO .net.xml file. Not required if sumo_cfg_file is provided.
#         route_file (Optional[str]): SUMO .rou.xml file. Not required if sumo_cfg_file is provided.
#         sumo_cfg_file (Optional[str]): SUMO .sumocfg configuration file. If provided, net_file and route_file are ignored.
#         out_csv_name (Optional[str]): name of the .csv output with simulation results. If None, no output is generated
#         use_gui (bool): Whether to run SUMO simulation with the SUMO GUI
#         virtual_display (Optional[Tuple[int,int]]): Resolution of the virtual display for rendering
#         begin_time (int): The time step (in seconds) the simulation starts. Default: 0
...
#         if sumo_cfg_file is not None:
#             assert os.path.exists(sumo_cfg_file), f"SUMO config file {sumo_cfg_file} not found"
#             self._sumo_cfg = sumo_cfg_file
#             self._net = None
#             self._route = None
#             self.use_config_file = True
#         else:
#             assert net_file is not None and route_file is not None, "Either sumo_cfg_file or both net_file and route_file must be provided"
#             assert os.path.exists(net_file), f"Network file {net_file} not found"
#             assert os.path.exists(route_file), f"Route file {route_file} not found"
#             self._net = net_file
#             self._route = route_file
#             self._sumo_cfg = None
...
#         # Initialize SUMO to get traffic light information
#         if LIBSUMO:
#             if self.use_config_file:
#                 traci.start([sumolib.checkBinary("sumo"), "-c", self._sumo_cfg])
#             else:
#                 traci.start([sumolib.checkBinary("sumo"), "-n", self._net])
#             conn = traci
#         else:
#             if self.use_config_file:
#                 traci.start([sumolib.checkBinary("sumo"), "-c", self._sumo_cfg], label="init_connection" + self.label)
#             else:
#                 traci.start([sumolib.checkBinary("sumo"), "-n", self._net], label="init_connection" + self.label)
#             conn = traci.getConnection("init_connection" + self.label)
...
#             # Override certain parameters if specified
#             if self.max_depart_delay != -1:
#                 sumo_cmd.extend(["--max-depart-delay", str(self.max_depart_delay)])
#             if self.waiting_time_memory != 1000:
#                 sumo_cmd.extend(["--waiting-time-memory", str(self.waiting_time_memory)])
#             if self.time_to_teleport != -1:
#                 sumo_cmd.extend(["--time-to-teleport", str(self.time_to_teleport)])
#         else:
#             # Use separate network and route files
#             sumo_cmd = [
#                 self._sumo_binary,
#                 "-n",
#                 self._net,

...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Multi-Agent DQN for OSM Traffic Signal Control"
    )
    ...
    parser.add_argument("-epsilon_decay", dest="epsilon_decay", type=float, default=0.995,
    ...
    out_csv_name = os.path.join(
        output_base_dir,
        f"dqn_lr{args.learning_rate}_gamma{args.gamma}_eps{args.epsilon}_decay{args.epsilon_decay}"
    )
    ...
    # Create DQN agents for each traffic signal
    agents = {}
    for ts_id in env.ts_ids:
        agents[ts_id] = DQNAgent(
    ...
    print(f"Created {len(agents)} DQN agents")
    ...
    for episode in range(args.episodes):
        ...
        if (episode + 1) % args.save_freq == 0:
            for ts_id in agents.keys():
                model_path = os.path.join(model_dir, f"dqn_agent_{ts_id}_episode_{episode + 1}.pt")
    ...
    print("\n=== Training Completed ===")
    for ts_id in agents.keys():
        model_path = os.path.join(model_dir, f"dqn_agent_{ts_id}_final.pt")
        agents[ts_id].save_model(model_path)
